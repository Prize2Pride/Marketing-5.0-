...
import { Send, Trash2, Bot, User, Loader2, BookOpen, Braces, FileText, ImageIcon, Clapperboard, Sparkles, Table2 } from "lucide-react";
...
export default function AITutor() {
  ...
  const [mode, setMode] = useState<"tutor" | "lesson" | "book" | "document" | "code" | "spreadsheet" | "infographic" | "video_brief" | "quiz">("tutor");
  ...
  const artifactMutation = trpc.creator.createArtifact.useMutation();
  ...
  const handleSend = () => {
    if (!input.trim() || chatMutation.isPending || artifactMutation.isPending) return;
    const msg = input.trim();
    setInput("");
    if (mode !== "tutor") {
      artifactMutation.mutate(
        { kind: mode, title: msg.slice(0, 160), prompt: msg, language },
        {
          onSuccess: () => {
            utils.creator.listMine.invalidate();
            toast.success(language === "ar" ? "تم حفظ المورد في مركز الإبداع" : language === "fr" ? "Ressource enregistrée dans le Hub" : "Artifact saved in Creator Hub");
          },
      ...
      <div className="flex-1 flex flex-col pt-16">
        <div className="container max-w-4xl flex-1 flex flex-col py-6 h-[calc(100vh-4rem)]">
          ...
          <div className="flex gap-2 overflow-x-auto pb-3 flex-shrink-0">
            {[
              { id: "tutor", label: language === "ar" ? "محادثة" : language === "fr" ? "Tutorat" : "Tutor", icon: Bot },
              { id: "lesson", label: language === "ar" ? "درس" : language === "fr" ? "Leçon" : "Lesson", icon: BookOpen },
              { id: "book", label: language === "ar" ? "كتاب" : language === "fr" ? "Livre" : "Book", icon: FileText },
              { id: "code", label: "Code", icon: Braces },
              { id: "spreadsheet", label: language === "ar" ? "جدول" : language === "fr" ? "Tableur" : "Sheet", icon: Table2 },
              { id: "infographic", label: language === "ar" ? "إنفوجرافيك" : language === "fr" ? "Infographie" : "Visual", icon: ImageIcon },
              { id: "video_brief", label: language === "ar" ? "فيديو" : language === "fr" ? "Vidéo" : "Video", icon: Clapperboard },
              { id: "quiz", label: language === "ar" ? "اختبار" : language === "fr" ? "Quiz" : "Quiz", icon: Sparkles },
            ].map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => setMode(id as typeof mode)} className={`flex items-center gap-1.5 shrink-0 rounded-full border px-3 py-1.5 text-xs transition-colors ${mode === id ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}>
          ...
          <div className="flex-shrink-0">
            <div className="flex gap-3 glass-card rounded-2xl p-3">
              <input
                ...
                placeholder={mode === "tutor" ? t("ai.placeholder") : language === "ar" ? "اكتب ما تريد إنشاءه مع السياق والهدف…" : language === "fr" ? "Décrivez ce que vous souhaitez créer avec le contexte et l'objectif…" : "Describe what you want to create, including context and objective…"}
              ...
              <Button
                ...
                onClick={handleSend}
                disabled={!input.trim() || chatMutation.isPending || artifactMutation.isPending}
              >
                {artifactMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
              </Button>
            </div>
          </div>
        </div>
