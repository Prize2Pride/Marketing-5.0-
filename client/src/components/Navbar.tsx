...
import { Globe, Menu, X, BookOpen, Bot, LayoutDashboard, Crown, GraduationCap, School, Sparkles, WandSparkles } from "lucide-react";
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { href: "/academy", label: language === "ar" ? "الأكاديمية" : language === "fr" ? "Académie" : "Academy", icon: GraduationCap },
    { href: "/ai-tutor", label: t("nav.aiTutor"), icon: Bot },
    ...(isAuthenticated ? [{ href: "/studio", label: t("nav.creator"), icon: Sparkles }] : []),
    ...(isAuthenticated ? [{ href: "/course-builder", label: language === "ar" ? "منشئ الدورات" : language === "fr" ? "Créateur de cours" : "Course Builder", icon: WandSparkles }] : []),
    ...(isAuthenticated ? [{ href: "/dashboard", label: t("nav.dashboard"), icon: LayoutDashboard }] : []),
    ...(user?.role === "educator" || user?.role === "admin" ? [{ href: "/teach", label: language === "ar" ? "فضاء المعلم" : language === "fr" ? "Espace enseignant" : "Teach", icon: School }] : []),
